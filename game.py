...
CHASER_LIFETIME = 3000  # chase 패턴이 터지기 전 생존 시간 (ms)
...
while running:
    ...
    for enemy in enemies:
        if enemy['pattern'] == 'straight':
            ...
            enemy['x'] += enemy['x_dir'] * enemy['x_speed'] * enemy_speed // 2
            # 벽에 닿으면 방향 반전
            ...
            if not enemy['exploded']:
                ...
                # 터질 시간 도달하면 explode
                if current_time - enemy['born_time'] > CHASER_LIFETIME or enemy['y'] > HEIGHT * 2 // 5:
                    enemy['exploded'] = True
                    enemy['fragments'] = create_fragments(enemy['x'], enemy['y'])
            else:
                # 파편 이동
                new_fragments = []
            ...
            if not enemy.get('exploded', False):
                ...
                if enemy['y'] > HEIGHT - 15:
                    ...
                    # [
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': -5, 'vy': -9.5, 'born_time': pygame.time.get_ticks()},
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': 5, 'vy': -9.5, 'born_time': pygame.time.get_ticks()},
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': -10.5, 'vy': -5.5, 'born_time': pygame.time.get_ticks()},
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': 10.5, 'vy': -5.5, 'born_time': pygame.time.get_ticks()},
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': 0, 'vy': -11, 'born_time': pygame.time.get_ticks()},
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': 11, 'vy': 0, 'born_time': pygame.time.get_ticks()},
                        # {'x': enemy['x'], 'y': enemy['y'], 'vx': -11, 'vy': 0, 'born_time': pygame.time.get_ticks()}
                    # ]
                ...
                new_fragments = []
                for frag in enemy['fragments']:
                    frag['x'] += frag['vx'] * enemy_speed / 5
                    frag['y'] += frag['vy'] * enemy_speed / 5
                    if current_time - frag['born_time'] < FRAGMENT_LIFETIME:
                        new_fragments.append(frag)
                enemy['fragments'] = new_fragments
